...
execute as @e[tag=kio.interactionorb,tag=!kio.falling,tag=!kio.rising] at @s if block ~ ~-1 ~ #minecraft:air run tag @s add kio.startfalling
...
execute as @e[tag=kio.rising] at @s if block ~ ~ ~ #air align xyz run tp @s ~.5 ~ ~.5
execute as @e[tag=kio.rising] at @s if block ~ ~ ~ #air run tag @s remove kio.rising